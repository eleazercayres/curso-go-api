...
func GetClientHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		http.Error(writer, fmt.Sprintf("Could not convert param to integer: %s", idParam), http.StatusMethodNotAllowed)
		return
	}

	// 1. Obter através do serviço de 'client' (proposta: GetClientByID) a entidade.
	...
	//		7.2 - Testar novo endpoint: curl http://localhost:9000/api/get-client\?id\=<id obtido na criação>
